import { Component, OnInit, AfterViewInit, ElementRef, Renderer2, ChangeDetectorRef, signal } from '@angular/core';
...
export class ContactUs implements OnInit, AfterViewInit {
  ...
  submitError = signal('');
  isInvestorPage = signal(false);
  formSubmitted = signal(false); // Track if form has been submitted
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(params => {
      this.isInvestorPage.set(params['type'] === 'investor');
    });
  ...
  onSubmit() {
    ...
    this.contactService.submitContactForm(this.formData).subscribe({
      ...
      error: (error) => {
        ...
        this.submitError.set(error.error?.message || 'Failed to submit form. Please try again.');
        this.isSubmitting.set(false);
      }
    });
  }
