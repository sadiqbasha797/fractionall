...
export class HomeComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  isCoowningVsRentalExpanded = signal<boolean>(false);
  isFaqExpanded = signal<boolean>(false);
  ...
  private simpleStepsTouchEndX = 0;
  private simpleStepsTouchEndY = 0;
    ...
    this.homeService.getPublicSimpleSteps().subscribe({
      next: (response) => {
        ...
        setTimeout(() => {
          ...
          this.startSimpleStepsAutoScroll();
        }, 0);
      },
      error: (error) => {
  ...
  loadBrands(): void {
    ...
    this.homeService.getPublicBrands().subscribe({
      next: (response) => {
        ...
        // Start brands auto-scroll after loading
        this.startBrandsAutoScroll();
      },
      error: (error) => {
  ...
  onResize() {
    ...
    // clamp brandsCurrentIndex to valid range after change
    const totalBrands = this.brands().length;
    if (totalBrands >= 2) {
      const maxBrandsIndex = Math.max(0, totalBrands - this.brandsVisibleCount());
      if (this.brandsCurrentIndex() > maxBrandsIndex) this.brandsCurrentIndex.set(maxBrandsIndex);
    } else {
  ...
  private updateBrandsVisibleCount() {
    if (!this.isBrowser) return;
    // Set to 4 brands for all screen sizes
    this.brandsVisibleCount.set(4);
  }
  ...
  prevBrands() {
    ...
    if (prevIndex < 0) {
      ...
      this.brandsCurrentIndex.set(prevIndex);
    }
  }
  ...
  nextBrands() {
    ...
    if (nextIndex > maxIndex) {
      ...
      this.brandsCurrentIndex.set(nextIndex);
    }
  }
  ...
  // Toggle section expand/collapse methods for mobile
  protected toggleWhyChooseFraction(): void {
    this.isWhyChooseFractionExpanded.set(!this.isWhyChooseFractionExpanded());
  }

  protected toggleTrustSecurity(): void {
    this.isTrustSecurityExpanded.set(!this.isTrustSecurityExpanded());
  }

  protected toggleCoowningVsRental(): void {
    this.isCoowningVsRentalExpanded.set(!this.isCoowningVsRentalExpanded());
  }

  protected toggleFaq(): void {
    this.isFaqExpanded.set(!this.isFaqExpanded());
  }

  // Simple steps carousel methods
  prevSimpleSteps() {
  ...
  // Get current step for carousel
  protected getCurrentStep(): SimpleStep | null {
    const steps = this.simpleSteps();
    if (steps.length === 0) return null;
    return steps[this.simpleStepsCurrentIndex()] || null;
  }
  ...
  private startBrandsAutoScroll(): void {
    ...
    }, this.brandsAutoScrollDelay);
  }

  private stopBrandsAutoScroll(): void {
    if (this.brandsAutoScrollInterval) {
      clearInterval(this.brandsAutoScrollInterval);
      this.brandsAutoScrollInterval = null;
    }
  }
  ...
  // Brands auto-scroll methods
  pauseBrandsAutoScroll(): void {
    this.isBrandsAutoScrollPaused.set(true);
  }

  resumeBrandsAutoScroll(): void {
    this.isBrandsAutoScrollPaused.set(false);
  }
  ...
  private startSimpleStepsAutoScroll(): void {
    ...
    }, this.simpleStepsAutoScrollDelay);
  }

  private stopSimpleStepsAutoScroll(): void {
    if (this.simpleStepsAutoScrollInterval) {
      clearInterval(this.simpleStepsAutoScrollInterval);
      this.simpleStepsAutoScrollInterval = null;
    }
  }

  pauseSimpleStepsAutoScroll(): void {
    this.isSimpleStepsAutoScrollPaused.set(true);
  }

  resumeSimpleStepsAutoScroll(): void {
    this.isSimpleStepsAutoScrollPaused.set(false);
  }
  ...
  // Mouse event handlers for simple steps carousel
  onSimpleStepsMouseEnter(): void {
    if (!this.isBrowser) return;
    this.pauseSimpleStepsAutoScroll();
  }

  onSimpleStepsMouseLeave(): void {
    if (!this.isBrowser) return;
    this.resumeSimpleStepsAutoScroll();
  }
  ...
  // Modal functions
  openSimpleStepsModal(): void {
    this.showSimpleStepsModal.set(true);
    document.body.style.overflow = 'hidden';
  }

  closeSimpleStepsModal(): void {
    this.showSimpleStepsModal.set(false);
    document.body.style.overflow = 'auto';
  }

  openWhyChooseModal(): void {
    this.showWhyChooseModal.set(true);
    document.body.style.overflow = 'hidden';
  }

  closeWhyChooseModal(): void {
    this.showWhyChooseModal.set(false);
    document.body.style.overflow = 'auto';
  }

  openTrustSecurityModal(): void {
    this.showTrustSecurityModal.set(true);
    document.body.style.overflow = 'hidden';
  }

  closeTrustSecurityModal(): void {
    this.showTrustSecurityModal.set(false);
    document.body.style.overflow = 'auto';
  }

  openCoowningVsRentalModal(): void {
    this.showCoowningVsRentalModal.set(true);
    document.body.style.overflow = 'hidden';
  }

  closeCoowningVsRentalModal(): void {
    this.showCoowningVsRentalModal.set(false);
    document.body.style.overflow = 'auto';
  }
