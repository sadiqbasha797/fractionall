...
export class PincodeService {
  ...
  getLocationByPincode(pincode: string): Observable<{city: string, state: string, district: string} | null> {
    ...
    return this.http.get<{city: string, state: string, district: string, formattedLocation: string}>(`${this.PINCODE_API_URL}/${pincode}`).pipe(
      map(response => {
        if (response && response.city && response.state) {
          return {
            city: response.city,
            state: response.state,
            district: response.district || response.city
          };
        }
        return null;
      }),
  ...
  getFormattedLocation(pincode: string): Observable<string> {
    ...
    if (this.PINCODE_MAPPING[pincode]) {
      const mapping = this.PINCODE_MAPPING[pincode];
      return of(`${mapping.city}, ${mapping.state}`);
    }
      ...
      map(response => {
        if (response && Array.isArray(response) && response.length > 0 && response[0].Status === 'Success') {
          ...
          if (postOfficeData && postOfficeData.length > 0) {
            const locationData = postOfficeData[0];
            return `${locationData.District || locationData.Name || ''}, ${locationData.State || ''}`;
          }
        }
        return '';
      }),
