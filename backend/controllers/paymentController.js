...
const processTokenRefund = async (req, res) => {
  try {
    ...
    const refundOptions = {
      payment_id: paymentId || token.razorpayPaymentId,
      amount: refundAmount ? refundAmount * 100 : token.amountpaid * 100, // Convert to paise
      notes: {
    ...
    token.refundDetails.refundAmount = refund.amount / 100; // Convert back from paise
